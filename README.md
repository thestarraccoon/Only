...
Предположим, что фронтенд обеспечивает передачу этого заголовка между запросами.

- Присутствует небольшая мультиязычность, используется для перевода ответов API. Доступные языки `ru/en`, по умолчанию `ru`.
